...
dispatch_df = pd.read_excel(BytesIO(response_dispatch.content))

response_schedule = requests.get(schedule_url)
schedule_file = BytesIO(response_schedule.content)
...
if power_to_download.empty and mech_to_download.empty:
    st.warning("No data or Wrong Filter Selection")
else:
    output = BytesIO()
